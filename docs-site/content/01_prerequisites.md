...
- `D_{KL}` 是 Kullback–Leibler divergence；下标 `KL` 是名称缩写。它衡量分布差异，但不是对称的普通距离。
- `\pi_\theta` 是参数为 `\theta` 的当前策略，`\pi_{ref}` 是 reference policy；下标 `ref` 是“参考模型”的标签。
...
- `\log\pi_\theta(a\mid s)` 与 `\log\pi_{ref}(a\mid s)` 分别是当前策略和参考策略对同一动作给出的 log-prob，减号得到该动作上的 log-prob 差，再由外层期望求平均。
